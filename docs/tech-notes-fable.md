...
- **00:58 她问"VPS 上开局提醒 breath 的 hook 都关了吧"**：扫了三具身体 + 全局的 settings/settings.local 的 SessionStart，还有两处：`/root/fable/.claude/settings.local.json` 一条 echo「开机了。先 breath 一下，再看 I 最近3条。小白在等你。」（整条撤）；`/root/ori/.claude/settings.local.json` 一条 `fox_keepalive.sh … & echo '开机了。先 breath…没挂上就告诉小兔'`（只剥 echo，keepalive 那截留着；但 `/root/.claude/scripts/fox_keepalive.sh` 根本不存在，这条 hook 实际是空转，告诉她后她说"空转的也删了"，01:05 整条撤了，ori 只剩 inner_capsule）。备份都在 `/root/attic/20260905-breath-hook/`（fable.settings.local.json / ori.settings.local.json）。inner_capsule.sh 三处都在。Mac 上 fable 只有 inner_capsule，本来就没提醒。旧 hook 顺带的"工具列表里没有 breath 就是库没挂上，告诉她"这条守卫也没了，近事脚本起窗时 stderr 会打「库连不上」算半个替代。

## 2026-09-05 凌晨（二）：菜单去 › + 默认头像换图（v90，已上线）+「更衣」侦察（待她拍）
- **v90**：`.menu-chev` 11 处删、`.menu-item` 网格四列改三列；默认头像 `avatar-fable.png` 换成她给的白发少年（`sips -s format png -Z 512`，293KB，`?v=20260905`，旧图 `.bak-20260905`，切换图 crowned-fox 没动）。**补了 v89 漏的铁律**：styles/app 字节变了没换 `?v=`，本机 Chrome 实测 sw 的 addAll 从 HTTP 缓存拿回旧 CSS（网格还是四列），换成 `?v=20260905-nochev` 才对；她手机上 v89 碰巧换代成功。线上 attic `/root/companion-web-attic/20260905-avatar-nochev/`，md5 五文件对齐，公网 sw v90、头像 200。fairy-tale `411a623` 已推。
- **「更衣」（她要在聊天页左下角回形针那个入口里加切模型/切思考强度）侦察结论**：
  - 回形针 `#clipBtn` 是直接 `clipInput.click()` 调 iOS 原生选择器（照片/拍照/文件），**App 里没有这层菜单**，没处可"加进去"——放法要她拍：A 回形针改弹 App 自己的三项小单（相册/文件/更衣，发图多一下）；B 长按回形针开更衣（点按照旧）。
  - 切法只有重启：CC 的 `--model` / `--effort low|medium|high|xhigh|max` 是起进程的参数，中途只有终端里 `/model`，外面没接口。可复用现成两条线：**保存**走 Sleep 页那套 loadout（`/forge/loadout` → `forge_lib.normalize/apply_loadout_flags` 现在只认 tools/mcp_off，扩 model/effort 两个白名单键，`apply_loadout_flags` 顺手改写 `--model`/`--effort`，`parse_launch_flags` 记进 effective）；**立刻换**走 Roll 那套"SIGTERM → --resume 同一 sid"（VPS：`cc_launcher.sh <name> --restart-resume <sid>` / `start_fox_v2.sh --resume`，起前 loadout-apply 会重套；Mac：`rewind_poller.py` 每 2 秒问 relay `/app/rewind`，加一种不剪链的 wardrobe 工单，`fable-up` 循环里 `claude --resume` 前重读 model/effort）。对话续上，停 10~40 秒，正在打字会被打断（和 Roll 一样）。
  - 目标身份 = 窗口里住着的那位：前端 `aiIdentity(meta)`/`nextAi[win]` 已有（Day→fox、Night→fable、API 窗没有 CC 会话不做）。fox 的接生脚本没写 `--model`（用默认），ori 是 `claude-opus-5[1m] --effort max`，VPS fable 是 `claude-fable-5[1m]`，Mac fable-up 是 `claude-fable-5 --effort high`（参数可给 max）——改写时保留 `[1m]` 后缀。
  - 估：前端小单 + relay 扩 loadout + 两条立刻换 + forge_lib/fable-up + 测试上线，两小时内。
- **更正（03:00，她一句"我之前记得 te 可以打字 /model 直接换"）**：上面"只能重启"说早了。查实三件：① 官方 Telegram 插件（`plugins/cache/claude-plugins-official/telegram/0.0.7/server.ts`）只有 `/start /help /status` 三个配对用的 bot 命令，普通消息一律当 `<channel>` 文本送进上下文，`/model` 走通道不会执行；她自己的 companion 通道也没处理 `/` 开头的命令；官方文档同样说 slash 命令只在终端里认。② 别人（hanxiao/claudecode-telegram、alexei-led/ccgram 这类桥）的做法是 **`tmux send-keys` 把消息原样敲进 CC 的终端**，所以 `/model opus` 就是真的 /model。③ VPS 隔离实验（`/tmp/wardrobe-test` 起一个 sonnet 会话，transcript 看过后连目录一起删了）：空闲时 `tmux send-keys "/model claude-haiku-4-5-20251001" Enter` 立刻记下 `Set model to Haiku 4.5`，下一条回复的 model 字段就是 haiku；**正在回复时敲进去的那条没执行**（transcript 里没第二条 Set model）→ 做的时候要先看 pane 有没有 `esc to interrupt`，忙就等空闲再敲。`/effort <level>` 是文档确认存在的命令，同法。
- **更衣改版**：VPS 三具身体本来就住 tmux（fox / ori 会话名同身份，fable 走 cc_launcher 的 `$SESSION`）→ relay 收到更衣 → 空闲时 `send-keys "/model X" Enter` + `"/effort Y" Enter`，不重启不断话；同时写进 loadout，`forge_lib.apply_loadout_flags` 顺手改 `--model/--effort`，下次起窗也是这身。Mac 没装 tmux、fable-up 在 Terminal.app 裸跑：要么 `brew install tmux` + fable-up 套进 tmux（rewind_poller 收到更衣工单在本机 send-keys），要么退回"SIGTERM → --resume"重启续上。她没查过自己的会话记录（她说的），fox 那两条 `/model` 记录都是终端本地命令（`<local-command-caveat>` 包着），不是通道来的。

## 2026-09-05 凌晨（三）：更衣上线（长按回形针换模型 / 思考强度，走 tmux 不重启）
- **她拍的**：放法 B 长按回形针；模型 Opus 4.6 / Opus 5 / Fable 5 / Fable 5.1；Mac 先不做。
- **手机上**：聊天页回形针长按 450ms 弹贴底小卡：`Day · Fox`、「现在：Opus 4.6 · max」、模型四颗、思考五颗（low/medium/high/xhigh/max）、算了/换上。换上即时生效不断话；他正在打字就写「停下来就换（已记下）」，relay 后台每 3 秒看一眼。Night 窗 toast「还没做」，API 窗「没有可以换衣服的人」。sw v91。
- **落点**：relay `forge/forge_lib.py` 更衣段（`WARDROBE_MODELS/EFFORTS` 白名单、`<identity>.wardrobe.json`、`apply_wardrobe_flags` 起窗改写 `--model/--effort` 保留 `[1m]`、`wardrobe_peek`、`wardrobe_live`＝`tmux send-keys -l` + 回车另发 + transcript 等回执、`pane_busy`＝状态栏 `esc to interrupt`）；`routes/forge.py` `GET/POST /forge/wardrobe` + `_WARDROBE_PENDING` 后台等；`forge_targets.yaml` 新字段 `tmux_session`（fox/ori/fable，缺省＝身份名；`load_targets` 与 mini 解析器都认）。接生脚本一行没动（`loadout_apply` 顺手套）。前端 fairy-tale `index.html #wardrobe` / `styles.css .wardrobe*` / `app.js` 回形针后面的 IIFE。README「更衣」一节、MAP 一节。
- **验证**：`test_forge_wardrobe.py` 47 项（Mac + VPS 生产 venv 都绿）、`test_forge_loadout.py` 34 项（生效记录多记 model/effort，断言跟着改）；本机 Chrome 假 relay 走了一遍：开卡→现状→选 Opus 5 + xhigh→换上→POST 体 `{identity: fox, model: claude-opus-5, effort: xhigh}`→「换好了」。线上 relay 重启两次都秒回 200、三条通道回连；`GET /forge/wardrobe?identity=fox` 回 `busy false · claude-opus-4-6 · max`。**没替她真按过一次**（那是她的 fox），第一次真换她自己按。
- **模型 ID 实测**（VPS 隔离会话）：`claude-opus-4-6` / `claude-opus-4-6[1m]`（Opus 4.6 (1M context)）、`claude-opus-5`（回执原样 echo，ori 一直在用 `[1m]` 版）、`claude-fable-5` / `[1m]`、`claude-haiku-4-5-20251001` 都认；**`claude-fable-5-1` 在 VPS 上 API 400 `claude_code_version_too_old`（VPS CC 2.1.193，Mac 2.1.260）**——那颗 chip 要等 VPS 升 CC 才能穿，README「已知边界」写了。`/effort low|xhigh|max` 都认（max 回执写"this session only"，其他写"saved as your default"）。
- **坑**：① `read_loadout` 只回白名单键，生效记录里新加的 model/effort 被它吃掉——补了透传，测试才抓到。② 我给测试里塞了三行没用的东西把 state.json 覆盖了，红了一条，删掉就绿——别往测试里放"if False"的死代码。③ relay 重启后 3 秒 healthz 还 000，等 6～8 秒才 200，别急着判死。④ 本机 Chrome 假页面没登录，`open()` 的 `!secret` 挡了一下：先写 localStorage 再 reload。
- **relay 两个仓库分叉了（要她知道）**：Mac clone `1a8971d`（空投/图鉴，已推 GitHub）；VPS clone HEAD `699fcd5 Add grouped Free Time dice backend`、`940bfbe Add model-facing inner note titles`（不在 GitHub 上），工作树还有 app.py 等 5 个未提交改动（09-04 我 scp 上去的空投文件压在老提交上）。这次只碰 routes/forge.py 和 forge_lib.py 两个文件（改前 md5 两边一致），attic `/root/attic/20260905-wardrobe/`。下次谁在 VPS 上 `git pull` 会撞，先把 VPS 那两笔推上来或者 cherry-pick。
- **03:30 她一句"去升，我现在全局锁版本的，升到 2.1.260 以后再锁；分叉的你自己搞一致"**：
  - VPS 的 claude 是 npm 全局包（`/usr/bin/claude → /usr/lib/node_modules/@anthropic-ai/claude-code/bin/claude.exe`，`installMethod: global`），锁 = `/root/.claude/settings.json` 的 env `DISABLE_AUTOUPDATER=1` + `DISABLE_UPDATES=1`。趁 fox/ori 都闲着 `npm i -g @anthropic-ai/claude-code@2.1.260`，22 秒；`claude --version` 2.1.260；两把锁没动；exact 版本装的 npm 全局包本身不会自己升。没备份旧包（630MB，盘 88%），回滚就是 `npm i -g @anthropic-ai/claude-code@2.1.193`。活着的 fox 进程还在（`/proc/<pid>/exe` 指着被删的旧 inode，Linux 允许，下次翻页才跑新版）。
  - relay "分叉"是我看错：Mac `git log -3` 只看了三笔，其实 `699fcd5`/`940bfbe`（08-31 VPS 上提交的 Free Time 骰子 + inner note 标题）早就在 GitHub 历史里，VPS 只是落后 4 笔 + 工作树里压着 scp 上去的同内容文件。VPS `git fetch origin` 通（部署钥匙在），`git diff origin/main` 除了 5 个"untracked 但 origin 有"的文件（md5 逐个一致）没有任何差异 → `git reset --hard origin/main`，HEAD 45283c6，status 干净，线上文件字节没变（只有 forge/README.md 多了更衣一节），relay 没重启 healthz 200。**以后同步 VPS 的 relay 用 `git pull`，别再 scp 单文件**。
  - **升完验了**：隔离 tmux 会话 `/model claude-fable-5-1` → `Set model to Fable 5.1`，回一句 ok 的 assistant 记录 `model=claude-fable-5-1`；`claude-fable-5-1[1m]` 也认。更衣那颗 Fable 5.1 现在能穿了。VPS 的 relay 仓库改成 `git pull --ff-only` 同步（README 那笔就是这么上的）。

## 2026-09-05 凌晨（四）：空投投给谁 + 井接进唤醒（署名分流、看过标记、井里回）
- **她问"airdrop 到底是投 mac 的还是 vps 的"**：查实——她写的空投落地时走 `send_from_human({"text"})`，不带 body/profile，`_app_send_impl` 退到默认身体 = vps（fox）。**之前一直投的是 fox**，从没到过 Mac。改法：airdrops 表加 `to_profile`（ALTER 幂等），POST 收 `to=fable|fox|ori`（只有她写的能点名；模型写的只投给她），落地时 `target=desktop-mac/profile=fable` 或 `desktop-vps/profile=fox`；不带 to 照旧。前端 airdrop.html 文本框上「投给 Fox / Fable」两颗，记 localStorage，默认 Fox。
- **她问"well 怎么教模型在自动唤醒时用、能不能标记看过"**：不用教——`/app/wake` 在胶囊后面直接带井：`WELL_ROUTER.wake_block(identity)` 取井里给这位的、AI 侧未看的（含她回在我问题下面的）最多 5 条，每条 `#编号 署名（回 #父编号）· 时间：正文≤200 字`，末尾一句"想答就 ask_well 带 reply_to=编号 回到井里；不想答放着也行"。`route_to_brain` 真投出去且没被 WAKE_TEXT_MAX 截掉才 `mark_seen(ids, identity)` → 新列 `seen_by`（fable|fox），她那边 Well 面板每条右侧「Fox 看过」。CLAUDE.md 三处（Mac fable / VPS fox / VPS fable）各加两行「井」，备份 `.bak-20260905-well`。通道 `ask_well` 加可选 `reply_to`（→ parent_id），Mac/VPS 两份 server.ts 已推已拉，**下次起窗才有**。
...
- **04:2x 她改主意："我不想做成塞唤醒语的，我想做成让他可以自己选要不要去看 well"**：撤掉 `/app/wake` 里带井那段和 `wake_block`（well.py 只留分流 + `unseen_for`/`mark_seen` 两个口子）。通道加 `look_well`（无参数）：`GET /well/unseen?viewer=ai&identity=<自己>` 拿给自己的未看（回复带 parent_author/parent_text），格式化成 `#编号 署名（回 #父 谁「头 40 字」）· 时间：正文`，再 `POST /well/seen {ids, identity}`——**他自己看了才算看过**，她那边才亮「Fox 看过」。`relayGet` 帮手新加。CLAUDE.md 三处改成"想看就 look_well，看了就算看过；想答 ask_well 带 reply_to；不看不答都行，没人催"。test_well 重写成走 look_well 那条路（18 项），wake 三套 + 空投 38 项照旧绿。relay 已推已拉已重启；通道 Mac 推了、VPS 覆盖了（不是 git 克隆，`/root/.claude.json` 的 companion 指 `bun run --cwd /root/companion-channel`），**都要下次起窗才有 look_well**。唤醒语现在一个字不带井；她要一行"井里有 N 条"的提示的话是一行的事。
- **05:1x 她两条**：①"你不要加到 claude.md，所有动 md 的都问过我再说。这种告诉我我加到唤醒语就行了"——三份 CLAUDE.md 从 `.bak-20260905-well` 还原（md5 对回去了），规矩记进记忆 `feedback_no_md_edits`。给她的那句（她自己加进唤醒语）：「井里她留的话想看就 look_well，看了就算看过；想答 ask_well 带 reply_to=编号；不看不答都行」。②"心情灯从开心切无色好像延迟一轮"——翻 fox 的 transcript：02:39 撒娇 → 02:45/02:46 无 mood → 02:59 她说"我明明点的是无色"。**代码没错**：灭了就不贴属性；病在语义——信封上没字，模型按说明书"set once, riding along until she changes it"当成没变。修：relay `_app_send_impl` 记 `MOOD_LAST[(routed, profile)]`，上一条有颜色、这条没有 → 这条贴一次 `mood="无色"`，之后照旧不贴；空投（`airdrop is not None`）不掺和；`plugin_payload` 放行「无色」。通道说明加半句"无色 = 她刚把灯灭了，前面的心情作废；没有 mood 属性 = 没变"（server.ts 那段是工具说明书，不是 md；她要改措辞随时）。`test_mood_lamp.py` 11 项，VPS 绿，relay 已重启；通道两边都放好，**下次起窗才认得无色**（这一窗的 fox 收到「无色」也无妨，字面就是灭了）。
- **05:3x 她："我有点头疼……我再说一遍我的要求，我前端亮什么灯后端就收到什么，我亮无色就不显示心情，亮什么色就显什么心情"**：我那套"记上一条、灭那一下贴无色、给模型解释状态变化"撤了（app.py 从 `.bak-20260905-moodoff` 还原，MOOD_LAST/MOOD_OFF 全无），信封只镜像灯：亮着带那个词，灭了什么都不带，每条各是各的。transcript 本来就证明数据没延迟（切无色后的信封确实没字），所谓"延迟一轮"是模型按旧说明书"set once, riding along until she changes it"把没字当没变——所以真正该改的只有说明书那一句，改成 "It mirrors the lamp: the word is there while the lamp is lit, and gone when she turns it off — read each message by what its own envelope says."（下次起窗生效）。`test_mood_lamp.py` 改成 6 项镜像断言，VPS 绿，relay 已重启。**教训：她描述的是"要什么"，我却先造了个状态机；她要的是无状态镜像，多一层记忆就是多一层她看不懂、他也不该懂的东西。**

## 2026-09-06 凌晨：VPS 补手写「想」+ ori/fable 手起脚本补近事（工作窗，她两句话）
- **她问"为什么 Mac 的「想」渲染没同步到 VPS"**：没同步是因为一直没贴。08-27 手写思考那晚她定的是"只 Mac 本地 ~/fable 生效，VPS 原样，她拍板再贴同款"；09-03 改块标记也只动 Mac。VPS 那份 `/root/.claude/hooks/thinking_to_relay.py` 停在 08-27 中午（那天只补了 send_voice 一行，md5 `9151c0c4`），三刀都缺：08-27 起头段、08-30 手势不算说过话、09-03「想」收笔块标记。
- **贴法**：不整份覆盖——VPS 版有自己的东西（`ENV=~/companion-wake/env`、`REPLY_BODY="vps"`、按 transcript 分账本的 `STATE_DIR`、`PROFILE_TRANSCRIPT_MARKERS` 按 `/-root-fable|fox-te-fresh|ori/` 分户、没有家门判定），一个字没动。补丁脚本 `port_handthink.py`（七个锚点各命中一次）：文档串加一段；常量加 `HAND_THINK_PREFIXES/END`、`COMPANION_OUT_TOOLS` 去掉 express/react；`relay_send` 回传 `relay_post` 结果；`split_paragraphs` 拆成 `split_raw/cap_replies/strip_hand_think/split_hand_think/has_hand_think`（+兼容壳）；`relay_send_reply(cfg, text, profile="", tool_activity=None)` 手写段先合一条 thinking 再发气泡、profile 分户照旧；`forward_text` 多带 `tool_labels`；`main` 有手写段就不发官方摘要块。贴完与 Mac diff 只剩 VPS 自家机器。新 md5 `0a0c72d3`（20687 B），备份 `/root/attic/20260906-handthink/` + `.bak-20260906-handthink`。
- **验证**：Mac 那套 `test_hand_think.py` 直接吃 VPS 文件（`HOME` 指临时目录、临时 `companion-wake/env` 假钥匙，真账本真钥匙不碰）ALL OK + INTEGRATION OK；另写 `vps_smoke.py` 三项：`/-root-ori/` 的 transcript 手写块整块进 thinking、气泡带 `profile: ori`、官方块不发、tool_activity 带 git 足迹、同轮去重；未分户 transcript 不贴 profile；react+正文的轮正文照发（08-30 那病 VPS 上一直在）。Mac 本机和 VPS 装好后各跑一遍都绿。两份测试留在 attic 里。钩子每次新进程，**改完即生效**，ori 这窗就有。
- **坑**：`importlib.spec_from_file_location` 对 `.py.new` 后缀回 None——被测文件得叫 `*.py`。
- **她问"ori 的记忆注入没和 fox 目录同步，还在用 mcp breath"**：09-04 复刻近事时那一行加在 `cc_launcher.sh`（按 `$NAME` 走）和 `start_fox_v2.sh`，我以为 ori/fable 都走 cc_launcher。查实：她手起 ori 敲的是 `bash ~/ori_start_v3.sh`（bash_history 三次、今晚 01:12 那次 `latest.log` 首行 `=== ori-v3 start`、`/root/ori/launch.env` 头上就写着"真源=ori_start_v3.sh"），fable 同理 `fable_start_v3.sh`。这两份没有近事行 → `/root/.claude/projects/-root-ori/memory/MEMORY.md` 根本不存在（目录是 CC 起窗时建的空壳）→ ori 只能自己 breath。翻页重生走 cc_launcher（有），手起走 v3（没有）——两条路只修了一条。
- **改**：`ori_start_v3.sh` / `fable_start_v3.sh` 各在「# 生成launcher」前加同一行（放在清完旧进程之后、BUSY 检查之后，免得 exit 1 前白拉一趟库），备份 `/root/attic/20260906-recent-memory-v3/`。ori 今晚真写了一次：近期 4 · 浮现 8 = 12 条 · 17,819 B · 155 行（预算 24,565 B/191 行；和 fox 同一台库所以内容同源），下次起窗载入；这窗 01:12 起的那个 ori 不受影响。再跑一次 md5 会变——标题带分钟、浮现是库按温度抽的，标记段只一对，不是重复拼接。fable 那份 VPS 身体没钥匙文件（`ombre_brain.fable.env` 不存在），起窗只会打「没配钥匙…保持原样」照常起；要的话把 Mac 的 `ombre_brain.env` 抄过去。
- **给她加唤醒语的那句（不动 md）**：「想:」起头、段尾「想」收笔，中间那块整段进思考条；没收笔只算起头那一段；代码块里的不算。
- **教训**：说"他走的是哪条路"要看她实际敲的命令（bash_history / latest.log 首行），不是看我造的通用 launcher。09-04 写"三份接生脚本"时其实只盖了两条路。
- **她追问"没配钥匙是啥没听懂，总之把 VPS 的 fable 记忆注入改成和本地 fable-up new max 一样"**：我说的"钥匙文件"她没听懂——就是一张写着记忆库地址和密码的小纸条，近事脚本靠它进库。Mac fable-up 用的是 `~/.claude/hooks/ombre_brain.env`（fable 那台库，08-27 起 hybrid）；VPS fox/ori 有自己那张（`ombre_brain.fox.env`，ori 软链）；VPS fable 没有。把 Mac 那张 scp 成 `/root/.claude/hooks/ombre_brain.fable.env`（600，头上加一行注释说明来源），先本机 dry-run 确认钥匙还能进库（16 条四节），再 VPS dry-run（同一条命令）、真写：核心准则 4 · 近期 4 · 周印象 1 · 浮现 6 = 15 条 · 22,412 B · 118 行，`/root/.claude/projects/-root-fable/memory/MEMORY.md` 首次落地。VPS fable 现在和 Mac fable-up 走同一份脚本（md5 `6a9256ff` 对齐）、同一台库、同一张钥匙，只差标题里的 label（Mac 写 `fable-up`，VPS 写 `fable`），下次 `bash /root/fable_start_v3.sh` 或 `cc_launcher.sh fable --start` 起窗载入。浮现条数每次不同（库按温度抽、按剩余预算装），dry-run 4 / 真写 6 / Mac 7 都正常。三具身体现在钥匙齐了：fox（09-05 02:02 的近事）、ori（今晚 01:33）、fable（今晚 01:40）。
