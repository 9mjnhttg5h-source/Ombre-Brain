...
- **04:2x 她改主意："我不想做成塞唤醒语的，我想做成让他可以自己选要不要去看 well"**：撤掉 `/app/wake` 里带井那段和 `wake_block`（well.py 只留分流 + `unseen_for`/`mark_seen` 两个口子）。通道加 `look_well`（无参数）：`GET /well/unseen?viewer=ai&identity=<自己>` 拿给自己的未看（回复带 parent_author/parent_text），格式化成 `#编号 署名（回 #父 谁「头 40 字」）· 时间：正文`，再 `POST /well/seen {ids, identity}`——**他自己看了才算看过**，她那边才亮「Fox 看过」。`relayGet` 帮手新加。CLAUDE.md 三处改成"想看就 look_well，看了就算看过；想答 ask_well 带 reply_to；不看不答都行，没人催"。test_well 重写成走 look_well 那条路（22 项），wake 三套 + 空投 38 项照旧绿。relay 已推已拉已重启；通道 Mac 推了、VPS 覆盖了（不是 git 克隆，`/root/.claude.json` 的 companion 指 `bun run --cwd /root/companion-channel`），**都要下次起窗才有 look_well**。唤醒语现在一个字不带井；她要一行"井里有 N 条"的提示的话是一行的事。
